  ...
  function getHabitsForSlide(date) {
    ...
    for (let i = 0; i < habits.length; i++) {
      ...
      if (moment(current_slide_date).isSame(start_date)) {
        plainOptions.plainOption.push({
          label: habits[i].habit_name,
          value: habits[i].habit_name,
        });
        ...
        for (let b = 0; b < habits[i].marked.length; b++) {
          if (moment(current_slide_date).isSame(habits[i].marked[b].date)) {
            ...
            console.log(
              "MARKIERT MUSS SEIN Date " +
                current_slide_date +
                " found: " +
                plainOptions.marked[plainOptions.marked.length - 1]
            );
          }
        }
      ...
      while (moment(start_date).isBefore(current_slide_date)) {
        start_date = start_date
          .add(habits[i].frequency_in_days, "days")
          .format("YYYY-MM-DD");
        ...
        if (moment(current_slide_date).isSame(start_date)) {
          ...
          for (let b = 0; b < habits[i].marked.length; b++) {
            if (moment(current_slide_date).isSame(habits[i].marked[b].date)) {
              plainOptions.marked.push(habits[i].habit_name);
              console.log(
                "Date " + current_slide_date + " found: " + plainOptions.marked
              );
            }
          }
          continue;
